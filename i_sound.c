...
{
/*
    if(HEXEN_BETA_DEMO || HEXEN_BETA)
    {
	if(	sfxinfo == SFX_FIREBALL			||	// not available in Retail Store Beta
		sfxinfo == SFX_SORCERER_BIGBALLEXPLODE	||	// not available in Retail Store Beta
		sfxinfo == SFX_SORCERER_HEADSCREAM	||	// not available in Retail Store Beta
		sfxinfo == SFX_PUPPYBEAT		||	// not available in Retail Store Beta
		sfxinfo == SFX_MYSTICINCANT		)	// not available in Retail Store Beta

		goto dont_play_sound;
//	else if(id) id = _addSfx( id, data, vol, steptable[pitch], sep );
    }
//    else if(HEXEN_MACDEMO || HEXEN_MACFULL || HEXEN_1_0 || HEXEN_1_1 || HEXEN_DEMO) id = _addSfx( id, data, vol, steptable[pitch], sep );
*/
    if (sound_module != NULL)
    {
        CheckVolumeSeparation(&vol, &sep);
