...
int Control_DirList(void)
{
	...
	if (dlist[dlist_curpos].type & TYPE_DIR) 
	{
	    ...
	    {
		strcat(hexen_wad_dir, dlist[dlist_curpos].name);
		strcat(hexen_wad_dir, "/");
...
*/
		    if (strncmp(iwad_term, temp, 4) == 0 && (fsize != 4429700 && fsize != 4440584))
		    {
			strcpy(target, path_tmp);
...
/*
	...
	if (dlist[dlist_curpos].type & TYPE_FILE)
	{
	    ...
	    strcpy(dehacked_file, hexen_wad_dir);
...
//	strcpy(dehacked_file, "");
	...
	if (dlist[dlist_curpos].type & TYPE_FILE)
	{
		...
		while (fgets(line, sizeof(line), file))
		{
		    ...
		    {
			strcpy(extra_wad, path_tmp);

			return 3;
		    }
		    else if (strncmp(iwad_term, temp, 4) == 0 && (fsizedd == 4429700 ||
								  fsizedd == 4440584)) 
		    {
			strcpy(extra_wad, path_tmp);
...
void displayFile()
{
	...
	if (WADLoaded == 0)
	{
	    ...
	    if(fsize == 10615976)
	    {
		mh_print(47, 111, "HEXEN IWAD VERSION: HEXEN 4-LEVEL-BETA DETECTED",
			rgb2col(255, 0, 0), 0, 0);

		mh_print(318, 111, "(THIS WAD IS SUPPORTED)",
			rgb2col(0, 255, 0), 0, 0);

		mh_print(47, 125, "YOU CANNOT -FILE WITH THE BETA & THE DEMO VERSIONS OF HEXEN. PLEASE REGISTER!",
			rgb2col(255, 0, 0), 0, 0);
	    }
	    else if(fsize == 20428208)
	    {
		mh_print(47, 111, "HEXEN IWAD VERSION: HEXEN BETA #3 DETECTED",
			rgb2col(255, 0, 0), 0, 0);
	    ...
	    {
		mh_print(47, 111, "HEXEN IWAD VERSION: HEXEN 4-LEVEL-DEMO DETECTED",
			rgb2col(255, 0, 0), 0, 0);
	    ...
	    {
		mh_print(47, 111, "HEXEN IWAD VERSION: HEXEN MAC DEMO DETECTED",
			rgb2col(255, 0, 0), 0, 0);
	    ...
	    {
		mh_print(47, 111, "HEXEN IWAD VERSION: HEXEN MAC REGISTERED DETECTED",
			rgb2col(255, 0, 0), 0, 0);
	    ...
	    {
		mh_print(47, 111, "HEXEN IWAD VERSION: HEXEN REGISTERED v1.0 DETECTED",
			rgb2col(255, 0, 0), 0, 0);
	    ...
	    {
		mh_print(47, 111, "HEXEN IWAD VERSION: HEXEN REGISTERED v1.1 DETECTED",
			rgb2col(255, 0, 0), 0, 0);

		mh_print(318, 111, "(THIS WAD IS SUPPORTED)",
			rgb2col(0, 255, 0), 0, 0);
	    }
	    else if(fsize != 10615976 && fsize != 20428208 && fsize != 10644136 && fsize != 13596228 &&
		    fsize != 21078584 && fsize != 20128392 && fsize != 20083672)
	    {
...
void drawDirectory()
{
    ...
    strncpy(path_tmp, hexen_wad_dir, 40);

    mh_print(0, 0,
    "                                  PSP-HEXEN RELEASE 1 BY NITR8                                  ",
	    rgb2col(255, 255, 0), 80000, 75);

    mh_print(47, 12, "SELECT THE GAME MAIN IWAD. YOU CAN ALSO SELECT AN OPTIONAL PWAD FILE TO LOAD.",
	    rgb2col(255, 255, 255), 0, 0);

//    mh_print(47, 67, "---------", rgb2col(255, 255, 255), 0, 0);
    mh_print(47, 63, "BUTTONS:", rgb2col(50, 255, 50), 0, 0);
...
//    mh_print(102, 63, "SQUARE  - select DEH file", rgb2col(255, 255, 255), 0, 0);
    mh_print(47, 73, "START   - launch the game", rgb2col(255, 255, 255), 0, 0);
    mh_print(47, 83, "CIRCLE  - clear entries", rgb2col(255, 255, 255), 0, 0);
...
*/
    ...
    mh_print(47, 97, "! WARNING: LOADING IWAD'S AS PWAD, VICE VERSA OR NON-HEXEN WAD'S, MAY CRASH !",
	    rgb2col(255, 0, 0), 0, 0);
...
void enterMenu(char *path)
{
    ...
    strcat(hexen_wad_dir, "WADS/");

    Get_DirList(hexen_wad_dir);
