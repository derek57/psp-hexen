...
void R_InitBuffer(int width, int height)
{
    ...
    for (i = 0; i < height; i++)
        ylookup[i] = screens[0]/*I_VideoBuffer*/ + (i + viewwindowy) * SCREENWIDTH;
}
...
void R_DrawViewBorder(void)
{
    ...
    {
        V_DrawPatch(x, viewwindowy - 4, 0, W_CacheLumpName("bordt", PU_CACHE));
        V_DrawPatch(x, viewwindowy + viewheight, 0, W_CacheLumpName("bordb",
                                                                 PU_CACHE));
    }
    for (y = viewwindowy; y < viewwindowy + viewheight; y += 16)
    {
        V_DrawPatch(viewwindowx - 4, y, 0, W_CacheLumpName("bordl", PU_CACHE));
        V_DrawPatch(viewwindowx + viewwidth, y, 0, W_CacheLumpName("bordr",
                                                                PU_CACHE));
    }
    V_DrawPatch(viewwindowx - 4, viewwindowy - 4, 0, W_CacheLumpName("bordtl",
                                                                  PU_CACHE));
...
*/
    ...
    if (viewwindowy < 35)
    {
        ...
        V_DrawPatch(viewwindowx - 4, viewwindowy, 0, W_CacheLumpName("bordl",
                                                                  PU_CACHE));
