...
//    I_AtExit(D_HexenQuitMessage, false);
    ...
    FILE *fprw = fopen("./psphexen.wad","rb");
	...
	printf("           PLEASE COPY THE FILE 'PSPHEXEN.WAD' WHICH CAME           ");
	...
	printf("\n\n\n\n\n\n\n\n\n\n\n\n\n");
	printf(" ================================================================== ");
	printf("      WARNING: RESOURCE PWAD FILE 'PSPHEXEN.WAD' IS MISSING!!!      ");
...
//        if (!netgame)
        {
            V_DrawPatch(160, viewwindowy + 5, 0, W_CacheLumpName("PAUSED",
                                                              PU_CACHE));
        }
/*
        else
...
static void PageDrawer(void)
{
    ...
    if (demosequence == 1)
    {
        V_DrawPatch(4, 160, 0, W_CacheLumpName("ADVISOR", PU_CACHE));
    }
...
static void CreateSavePath(void)
{
    ...
    char creationRoot1[121];
    char creationRoot2[121];
    char creationRoot3[121];

    strcpy(creationRoot1, SavePathRoot1);
    strcpy(creationRoot2, SavePathRoot2);
    strcpy(creationRoot3, SavePathRoot3);

//    creationRoot[120] = 0;

    mkdir( creationRoot1, S_IRWXU|S_IRWXG|S_IRWXO );
    mkdir( creationRoot2, S_IRWXU|S_IRWXG|S_IRWXO );
    mkdir( creationRoot3, S_IRWXU|S_IRWXG|S_IRWXO );

    if(!datadisc && HEXEN_BETA)
	strcpy(creationPath, SavePathBeta);

    if(!datadisc && HEXEN_BETA_DEMO)
	strcpy(creationPath, SavePathBetaDemo);

    if(!datadisc && HEXEN_MACDEMO)
	strcpy(creationPath, SavePathMacDemo);

    if(!datadisc && HEXEN_MACFULL)
	strcpy(creationPath, SavePathMacFull);

    if(!datadisc && HEXEN_DEMO)
	strcpy(creationPath, SavePathDemo);

    if(!datadisc && HEXEN_1_0)
	strcpy(creationPath, SavePath10);

    if(!datadisc && HEXEN_1_1)
	strcpy(creationPath, SavePath11);

    if(datadisc && HEXDD_1_0)
	strcpy(creationPath, SavePathDD10);

    if(datadisc && HEXDD_1_1)
	strcpy(creationPath, SavePathDD11);
