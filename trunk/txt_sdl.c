...
int TXT_Init(void)
{
    ...
    screen2 = SDL_SetVideoMode(TXT_SCREEN_W * font->w,
...
void TXT_Shutdown(void)
{
	...
	I_ShutdownGraphics ();
	I_ShutdownSound();
	Z_FreeMemory();			// ADDED FOR PSP SHUTDOWN
...
void TXT_UpdateScreenArea(int x, int y, int w, int h)
{
    ...
    SDL_BlitSurface(screenbuffer, &rect, screen2, &rect);
    SDL_UpdateRects(screen2, 1, &rect);
}
