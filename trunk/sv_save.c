...
#define GET_BYTE StreamInByte()
#define GET_WORD StreamInWord()
#define GET_LONG StreamInLong()
#define MAX_MAPS 99
#define BASE_SLOT 6
...
static void OpenStreamIn(char *fileName);
static void CloseStreamIn(void);
static void StreamInBuffer(void *buffer, int size);
byte StreamInByte();
short StreamInWord();
int StreamInLong();
...
void SV_SaveGame(int slot, char *description)
{
	...
	if(!datadisc && HEXEN_BETA) sprintf(fileName, "%shex6.hxs", SavePathBeta);
	if(!datadisc && HEXEN_BETA_DEMO) sprintf(fileName, "%shex6.hxs", SavePathBetaDemo);
	if(!datadisc && HEXEN_MACDEMO) sprintf(fileName, "%shex6.hxs", SavePathMacDemo);
	if(!datadisc && HEXEN_MACFULL) sprintf(fileName, "%shex6.hxs", SavePathMacFull);
	if(!datadisc && HEXEN_DEMO) sprintf(fileName, "%shex6.hxs", SavePathDemo);
	if(!datadisc && HEXEN_1_0) sprintf(fileName, "%shex6.hxs", SavePath10);
	if(!datadisc && HEXEN_1_1) sprintf(fileName, "%shex6.hxs", SavePath11);
	if(datadisc && HEXDD_1_0) sprintf(fileName, "%shex6.hxs", SavePathDD10);
	if(datadisc && HEXDD_1_1) sprintf(fileName, "%shex6.hxs", SavePathDD11);
...
void SV_SaveMap(boolean savePlayers)
{
	...
	// Open the output file
	if(!datadisc && HEXEN_BETA) sprintf(fileName, "%shex6%02d.hxs", SavePathBeta, gamemap);
	if(!datadisc && HEXEN_BETA_DEMO) sprintf(fileName, "%shex6%02d.hxs", SavePathBetaDemo, gamemap);
	if(!datadisc && HEXEN_MACDEMO) sprintf(fileName, "%shex6%02d.hxs", SavePathMacDemo, gamemap);
	if(!datadisc && HEXEN_MACFULL) sprintf(fileName, "%shex6%02d.hxs", SavePathMacFull, gamemap);
	if(!datadisc && HEXEN_DEMO) sprintf(fileName, "%shex6%02d.hxs", SavePathDemo, gamemap);
	if(!datadisc && HEXEN_1_0) sprintf(fileName, "%shex6%02d.hxs", SavePath10, gamemap);
	if(!datadisc && HEXEN_1_1) sprintf(fileName, "%shex6%02d.hxs", SavePath11, gamemap);
	if(datadisc && HEXDD_1_0) sprintf(fileName, "%shex6%02d.hxs", SavePathDD10, gamemap);
	if(datadisc && HEXDD_1_1) sprintf(fileName, "%shex6%02d.hxs", SavePathDD11, gamemap);
	OpenStreamOut(fileName);
...
void SV_LoadGame(int slot)
{
	...
	StreamInBuffer(&vertext, HXS_DESCRIPTION_LENGTH);
	StreamInBuffer(&vertext, HXS_VERSION_TEXT_LENGTH);
	if(strcmp(vertext, HXS_VERSION_TEXT))
	{ // Bad version
		return;
	}
	...
	StreamInBuffer(WorldVars,sizeof(WorldVars));
	StreamInBuffer(ACSStore,sizeof(ACSStore));
...
/*
	...
	oldWeaponowned[0] = 0;	// added for GCC > 4.3.1
	oldWeaponowned[1] = 0;	// added for GCC > 4.3.1
	oldWeaponowned[2] = 0;	// added for GCC > 4.3.1
	oldWeaponowned[3] = 0;	// added for GCC > 4.3.1

	int oldKeys   = 0; // added initialiser (avoid compiler warning)
	int oldPieces = 0; // added initialiser (avoid compiler warning)
	int bestWeapon;
...
boolean SV_RebornSlotAvailable(void)
{
	...
	if(!datadisc && HEXEN_BETA) sprintf(fileName, "%shex%d.hxs", SavePathBeta, REBORN_SLOT);
	if(!datadisc && HEXEN_BETA_DEMO) sprintf(fileName, "%shex%d.hxs", SavePathBetaDemo, REBORN_SLOT);
	if(!datadisc && HEXEN_MACDEMO) sprintf(fileName, "%shex%d.hxs", SavePathMacDemo, REBORN_SLOT);
	if(!datadisc && HEXEN_MACFULL) sprintf(fileName, "%shex%d.hxs", SavePathMacFull, REBORN_SLOT);
	if(!datadisc && HEXEN_DEMO) sprintf(fileName, "%shex%d.hxs", SavePathDemo, REBORN_SLOT);
	if(!datadisc && HEXEN_1_0) sprintf(fileName, "%shex%d.hxs", SavePath10, REBORN_SLOT);
	if(!datadisc && HEXEN_1_1) sprintf(fileName, "%shex%d.hxs", SavePath11, REBORN_SLOT);
	if(datadisc && HEXDD_1_0) sprintf(fileName, "%shex%d.hxs", SavePathDD10, REBORN_SLOT);
	if(datadisc && HEXDD_1_1) sprintf(fileName, "%shex%d.hxs", SavePathDD11, REBORN_SLOT);
...
static void UnarchivePlayers(void)
{
	...
	for(i = 0; i < MAXPLAYERS; i++)
	{
		...
		StreamInBuffer(&players[i],sizeof(player_t));
...
static void UnarchiveMobjs(void)
{
	...
	{
		mobj = MobjList[i];
		StreamInBuffer(mobj, sizeof(mobj_t));
...
static void RestoreMobj(mobj_t *mobj)
{
	...
	SetMobjPtr((int *)(void*)&mobj->target);
	switch(mobj->type)
	{
		// Just special1
		case MT_BISH_FX:
	...
	while((tClass = GET_BYTE) != TC_NULL)
	{
		for(info = ThinkerInfo; info->tClass != TC_NULL; info++)
		{
			if(tClass == info->tClass)
			{
				...
				StreamInBuffer(thinker, info->size);
...
static void RestoreScript(acs_t *script)
{
	...
	SetMobjPtr((int *)(void*)&script->activator);
}
...
static void UnarchiveScripts(void)
{
	...
	StreamInBuffer(MapVars,sizeof(MapVars));
}
...
static void ClearSaveSlot(int slot)
{
	...
	{
		if(!datadisc && HEXEN_BETA) sprintf(fileName, "%shex%d%02d.hxs", SavePathBeta, slot, i);
		if(!datadisc && HEXEN_BETA_DEMO) sprintf(fileName, "%shex%d%02d.hxs", SavePathBetaDemo, slot, i);
		if(!datadisc && HEXEN_MACDEMO) sprintf(fileName, "%shex%d%02d.hxs", SavePathMacDemo, slot, i);
		if(!datadisc && HEXEN_MACFULL) sprintf(fileName, "%shex%d%02d.hxs", SavePathMacFull, slot, i);
		if(!datadisc && HEXEN_DEMO) sprintf(fileName, "%shex%d%02d.hxs", SavePathDemo, slot, i);
		if(!datadisc && HEXEN_1_0) sprintf(fileName, "%shex%d%02d.hxs", SavePath10, slot, i);
		if(!datadisc && HEXEN_1_1) sprintf(fileName, "%shex%d%02d.hxs", SavePath11, slot, i);
		if(datadisc && HEXDD_1_0) sprintf(fileName, "%shex%d%02d.hxs", SavePathDD10, slot, i);
		if(datadisc && HEXDD_1_1) sprintf(fileName, "%shex%d%02d.hxs", SavePathDD11, slot, i);
		remove(fileName);
	}
	if(!datadisc && HEXEN_BETA) sprintf(fileName, "%shex%d.hxs", SavePathBeta, slot);
	if(!datadisc && HEXEN_BETA_DEMO) sprintf(fileName, "%shex%d.hxs", SavePathBetaDemo, slot);
	if(!datadisc && HEXEN_MACDEMO) sprintf(fileName, "%shex%d.hxs", SavePathMacDemo, slot);
	if(!datadisc && HEXEN_MACFULL) sprintf(fileName, "%shex%d.hxs", SavePathMacFull, slot);
	if(!datadisc && HEXEN_DEMO) sprintf(fileName, "%shex%d.hxs", SavePathDemo, slot);
	if(!datadisc && HEXEN_1_0) sprintf(fileName, "%shex%d.hxs", SavePath10, slot);
	if(!datadisc && HEXEN_1_1) sprintf(fileName, "%shex%d.hxs", SavePath11, slot);
	if(datadisc && HEXDD_1_1) sprintf(fileName, "%shex%d.hxs", SavePathDD10, slot);
	if(datadisc && HEXDD_1_1) sprintf(fileName, "%shex%d.hxs", SavePathDD11, slot);
	remove(fileName);
}
...
static void CopySaveSlot(int sourceSlot, int destSlot)
{
	...
	for(i = 0; i < MAX_MAPS; i++)
	{
		if(!datadisc && HEXEN_BETA) sprintf(sourceName, "%shex%d%02d.hxs", SavePathBeta, sourceSlot, i);
		if(!datadisc && HEXEN_BETA_DEMO) sprintf(sourceName, "%shex%d%02d.hxs", SavePathBetaDemo, sourceSlot, i);
		if(!datadisc && HEXEN_MACDEMO) sprintf(sourceName, "%shex%d%02d.hxs", SavePathMacDemo, sourceSlot, i);
		if(!datadisc && HEXEN_MACFULL) sprintf(sourceName, "%shex%d%02d.hxs", SavePathMacFull, sourceSlot, i);
		if(!datadisc && HEXEN_DEMO) sprintf(sourceName, "%shex%d%02d.hxs", SavePathDemo, sourceSlot, i);
		if(!datadisc && HEXEN_1_0) sprintf(sourceName, "%shex%d%02d.hxs", SavePath10, sourceSlot, i);
		if(!datadisc && HEXEN_1_1) sprintf(sourceName, "%shex%d%02d.hxs", SavePath11, sourceSlot, i);
		if(datadisc && HEXDD_1_0) sprintf(sourceName, "%shex%d%02d.hxs", SavePathDD10, sourceSlot, i);
		if(datadisc && HEXDD_1_1) sprintf(sourceName, "%shex%d%02d.hxs", SavePathDD11, sourceSlot, i);
		if(ExistingFile(sourceName))
		{
			if(!datadisc && HEXEN_BETA) sprintf(destName, "%shex%d%02d.hxs", SavePathBeta, destSlot, i);
			if(!datadisc && HEXEN_BETA_DEMO) sprintf(destName, "%shex%d%02d.hxs", SavePathBetaDemo, destSlot, i);
			if(!datadisc && HEXEN_MACDEMO) sprintf(destName, "%shex%d%02d.hxs", SavePathMacDemo, destSlot, i);
			if(!datadisc && HEXEN_MACFULL) sprintf(destName, "%shex%d%02d.hxs", SavePathMacFull, destSlot, i);
			if(!datadisc && HEXEN_DEMO) sprintf(destName, "%shex%d%02d.hxs", SavePathDemo, destSlot, i);
			if(!datadisc && HEXEN_1_0) sprintf(destName, "%shex%d%02d.hxs", SavePath10, destSlot, i);
			if(!datadisc && HEXEN_1_1) sprintf(destName, "%shex%d%02d.hxs", SavePath11, destSlot, i);
			if(datadisc && HEXDD_1_0) sprintf(destName, "%shex%d%02d.hxs", SavePathDD10, destSlot, i);
			if(datadisc && HEXDD_1_1) sprintf(destName, "%shex%d%02d.hxs", SavePathDD11, destSlot, i);
			CopyFile(sourceName, destName);
		}
	}
	if(!datadisc && HEXEN_BETA) sprintf(sourceName, "%shex%d.hxs", SavePathBeta, sourceSlot);
	if(!datadisc && HEXEN_BETA_DEMO) sprintf(sourceName, "%shex%d.hxs", SavePathBetaDemo, sourceSlot);
	if(!datadisc && HEXEN_MACDEMO) sprintf(sourceName, "%shex%d.hxs", SavePathMacDemo, sourceSlot);
	if(!datadisc && HEXEN_MACFULL) sprintf(sourceName, "%shex%d.hxs", SavePathMacFull, sourceSlot);
	if(!datadisc && HEXEN_DEMO) sprintf(sourceName, "%shex%d.hxs", SavePathDemo, sourceSlot);
	if(!datadisc && HEXEN_1_0) sprintf(sourceName, "%shex%d.hxs", SavePath10, sourceSlot);
	if(!datadisc && HEXEN_1_1) sprintf(sourceName, "%shex%d.hxs", SavePath11, sourceSlot);
	if(datadisc && HEXDD_1_0) sprintf(sourceName, "%shex%d.hxs", SavePathDD10, sourceSlot);
	if(datadisc && HEXDD_1_1) sprintf(sourceName, "%shex%d.hxs", SavePathDD11, sourceSlot);
	if(ExistingFile(sourceName))
	{
		if(!datadisc && HEXEN_BETA) sprintf(destName, "%shex%d.hxs", SavePathBeta, destSlot);
		if(!datadisc && HEXEN_BETA_DEMO) sprintf(destName, "%shex%d.hxs", SavePathBetaDemo, destSlot);
		if(!datadisc && HEXEN_MACDEMO) sprintf(destName, "%shex%d.hxs", SavePathMacDemo, destSlot);
		if(!datadisc && HEXEN_MACFULL) sprintf(destName, "%shex%d.hxs", SavePathMacFull, destSlot);
		if(!datadisc && HEXEN_DEMO) sprintf(destName, "%shex%d.hxs", SavePathDemo, destSlot);
		if(!datadisc && HEXEN_1_0) sprintf(destName, "%shex%d.hxs", SavePath10, destSlot);
		if(!datadisc && HEXEN_1_1) sprintf(destName, "%shex%d.hxs", SavePath11, destSlot);
		if(datadisc && HEXDD_1_0) sprintf(destName, "%shex%d.hxs", SavePathDD10, destSlot);
		if(datadisc && HEXDD_1_1) sprintf(destName, "%shex%d.hxs", SavePathDD11, destSlot);
		CopyFile(sourceName, destName);
	}
}
...
{
byte val;
	fread(&val, sizeof(byte), 1, SavingFP);
return val;
}
...
{
short val;
	fread(&val, sizeof(short), 1, SavingFP);
return val;
}
...
{
int val;
	fread(&val, sizeof(int), 1, SavingFP);
return val;
}
