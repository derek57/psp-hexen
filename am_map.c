...
boolean AM_Responder(event_t* ev)
{
    ...
    // ...FROM CHOCOLATE) WASN'T EASY TO HANDLE...
    // ...AS IT'S HARD FOR THE LOW AMOUNT OF KEYS...
    // ...ON THE PSP TO CONTROL AS MUCH AS POSSIBLE!!!
    //
    // SO I TOOK THE CODE FROM MY OLD PORT OF...
    // ...ENHANCED PSP-HEXEN AND CHANGED FEW LINES!!!
...
//    static int cheatstate=0;
	...
	if (ev->type == ev_keydown && ev->data1 == key_map_toggle &&
		gamestate == GS_LEVEL && button_layout == 0)
	{
	...
	else if (ev->type == ev_keydown && ev->data1 == key_map_toggle &&
		gamestate == GS_LEVEL && button_layout == 1 && gamekeydown[key_use])
	{
	    dont_move_backwards = true;
	    AM_Start ();
	    SB_state = -1;
...
/*
	    ...
	    case AM_FOLLOWKEY:
		    ...
		    f_oldloc.x = INT_MAX;
		    P_SetMessage(plr, 
			    followplayer ? TXT_AMSTR_FOLLOWON : TXT_AMSTR_FOLLOWOFF, true);
		    break;
*/
	    default:
	    	rc = false;
...
//                      dmapx=0;  //goes into the automap.
    ...
    {
        memcpy(screens[0]/*I_VideoBuffer*/ + i * finit_width, maplump + j + mapxstart,
               finit_width - mapxstart);
        memcpy(screens[0]/*I_VideoBuffer*/ + i * finit_width + finit_width - mapxstart,
               maplump + j, mapxstart);
        j += finit_width;
                          ...
                          angle_t angle, int color, fixed_t x, fixed_t y)
{
    ...
    if (automapactive & am_rotate) angle -= plr->mo->angle - ANG90; // cph
        ...
        while (t)
        {
  	    ...
  	    if (automapactive & am_rotate)
            ...
            AM_drawLineCharacter(thintriangle_guy, NUMTHINTRIANGLEGUYLINES,
                                 radius, t->angle, colors + lightlev,
                                 x, y);
            t = t->snext;
        }
    }
