...
#endif
*/
/*extern*/ int filesize_hexen_wad;
/*extern*/ int hexen_wad_exists;
/*
...
void W_CheckSize(int wad)
{
	...
	fpdd = fopen(path_tmp, "r");

	if (fpdd == NULL)
	    printf(" ");
	else
	{
	    fseek(fpdd, 0, 2);		// file pointer at the end of file
	    fsizedd = ftell(fpdd);	// take a position of file pointer un size variable
	    fclose(fpdd);
	}
    }
    else if(wad == 2)
	...
	{
	    fseek(fpdd, 0, 2);		// file pointer at the end of file
	    fsize = ftell(fpdd);	// take a position of file pointer un size variable
	    fclose(fpdd);
	}
    }
}
