...
static patch_t *PatchINVHBAR;
static patch_t *PatchMANAVIALHBAR1;
static patch_t *PatchMANAVIALHBAR2;
static patch_t *PatchMANAVIALDIMHBAR1;
static patch_t *PatchMANAVIALDIMHBAR2;
/*
// Toggle god mode
...
//    PatchINVLFGEM2 = W_CacheLumpName("invgeml2", PU_STATIC);
    ...
    PatchINVHBAR = W_CacheLumpName("INVHBAR", PU_STATIC);
    PatchMANAVIALHBAR1 = W_CacheLumpName("MNVLHBR1", PU_STATIC);
    PatchMANAVIALHBAR2 = W_CacheLumpName("MNVLHBR2", PU_STATIC);
    PatchMANAVIALDIMHBAR1 = W_CacheLumpName("MNVLHBR3", PU_STATIC);
    PatchMANAVIALDIMHBAR2 = W_CacheLumpName("MNVLHBR4", PU_STATIC);
...
static void DrINumber(signed int val, int x, int y)
{
    ...
    if (val < 0)
    {
        ...
        if (val > 9)
        {
            ...
            V_DrawPatch(x + 8, y, 0, patch);
            V_DrawPatch(x, y, 0, PatchNEGATIVE);
        }
        else
        {
            V_DrawPatch(x + 8, y, 0, PatchNEGATIVE);
        }
        val = val % 10;
        patch = PatchINumbers[val];
        V_DrawPatch(x + 16, y, 0, patch);
        return;
    }
    if (val > 99)
    {
...
static void DrawSoundInfo(void)
{
    ...
    if (leveltime & 16)
    {
        MN_DrTextA("*** SOUND DEBUG INFO ***", xPos[0], 45);
    }
    S_GetChannelInfo(&s);
    if (s.channelCount == 0)
    {
    ...
    x = 0;
    MN_DrTextA("NAME", xPos[x++], 55);
    MN_DrTextA("MO.T", xPos[x++], 55);
    MN_DrTextA("MO.X", xPos[x++], 55);
    MN_DrTextA("MO.Y", xPos[x++], 55);
    MN_DrTextA("ID", xPos[x++], 55);
    MN_DrTextA("PRI", xPos[x++], 55);
    MN_DrTextA("DIST", xPos[x++], 55);
...
void SB_Drawer(void)
{
	...
	V_DrawPatch(158, 82, 0, PatchXHAIR);
    ...
    else
    {
        if (SB_state == -1)
        {
            V_DrawPatch(0, 134, 0, PatchH2BAR);
            oldhealth = -1;
        }
        DrawCommonBar();
        if (!inventory)
        {
            if (SB_state != 0)
            {
                ...
                if (!automapactive)
                {
                    V_DrawPatch(38, 162, 0, PatchSTATBAR);
                }
                else
                {
                    V_DrawPatch(38, 162, 0, PatchKEYBAR);
                }
                oldarti = 0;
                oldmana1 = -1;
                oldmana2 = -1;
...
static void DrawAnimatedIcons(void)
{
    ...
    if (CPlayer->powers[pw_flight])
    {
            ...
            || !(CPlayer->powers[pw_flight] & 16))
        {
            ...
            if (CPlayer->mo->flags2 & MF2_FLY)
            {
                ...
                {
                    V_DrawPatch(20, 19, 0, W_CacheLumpNum(SpinFlylump + 15,
                                                       PU_CACHE));
                }
                else
                {
                    V_DrawPatch(20, 19, 0, W_CacheLumpNum(SpinFlylump + frame,
                                                       PU_CACHE));
    ...
    if (CPlayer->powers[pw_speed])
    {
        ...
        {
            frame = (leveltime / 3) & 15;
            V_DrawPatch(60, 19, 0, W_CacheLumpNum(SpinSpeedLump + frame,
                                               PU_CACHE));
        }
        BorderTopRefresh = true;
        UpdateState |= I_MESSAGES;
    ...
    if (CPlayer->powers[pw_invulnerability])
    {
        ...
        {
            frame = (leveltime / 3) & 15;
            V_DrawPatch(260, 19, 0, W_CacheLumpNum(SpinDefenseLump + frame,
                                                PU_CACHE));
        }
        BorderTopRefresh = true;
        UpdateState |= I_MESSAGES;
    ...
    if (CPlayer->powers[pw_minotaur])
    {
        ...
        {
            frame = (leveltime / 3) & 15;
            V_DrawPatch(300, 19, 0, W_CacheLumpNum(SpinMinotaurLump + frame,
                                                PU_CACHE));
        }
        BorderTopRefresh = true;
        UpdateState |= I_MESSAGES;
...
void DrawCommonBar(void)
{
    ...
    V_DrawPatch(0, 134, 0, PatchH2TOP);
    ...
    if (oldhealth != HealthMarker)
    {
        ...
        V_DrawPatch(28 + (((healthPos * 196) / 100) % 9), 193, 0, PatchCHAIN);
        V_DrawPatch(7 + ((healthPos * 11) / 5), 193, 0, PatchLIFEGEM);
        V_DrawPatch(0, 193, 0, PatchLFEDGE);
        V_DrawPatch(277, 193, 0, PatchRTEDGE);
//              ShadeChain();
...
void DrawMainBar(void)
{
    ...
    if (ArtifactFlash)
    {
        V_DrawPatch(144, 160, 0, PatchARTICLEAR);
        V_DrawPatch(148, 164, 0, W_CacheLumpNum(W_GetNumForName("useartia")
...
/*
    if (deathmatch)
    {
        ...
        if (temp != oldfrags)
        {
            V_DrawPatch(38, 162, 0, PatchKILLS);
...
*/
    {
        ...
        {
            oldlife = temp;
            V_DrawPatch(41, 178, 0, PatchARMCLEAR);
            if (temp >= 25)
            {
                DrINumber(temp, 40, 176);
            }
    ...
    if (oldmana1 != temp)
    {
        V_DrawPatch(77, 178, 0, PatchMANACLEAR);
    ...
    if (oldmana2 != temp)
    {
        V_DrawPatch(109, 178, 0, PatchMANACLEAR);
    ...
    {                           // Update mana graphics based upon mana count/weapon type
        ...
        V_DrawPatch(77, 164, 0, manaPatch1);
        V_DrawPatch(110, 164, 0, manaPatch2);
        V_DrawPatch(94, 164, 0, manaVialPatch1);
        for (i = 165; i < 187 - (22 * CPlayer->mana[0]) / MAX_MANA; i++)
        {
            screens[0]/*I_VideoBuffer*/[i * SCREENWIDTH + 95] = 0;
            screens[0]/*I_VideoBuffer*/[i * SCREENWIDTH + 96] = 0;
            screens[0]/*I_VideoBuffer*/[i * SCREENWIDTH + 97] = 0;
        }
        V_DrawPatch(102, 164, 0, manaVialPatch2);
        for (i = 165; i < 187 - (22 * CPlayer->mana[1]) / MAX_MANA; i++)
        {
            screens[0]/*I_VideoBuffer*/[i * SCREENWIDTH + 103] = 0;
            screens[0]/*I_VideoBuffer*/[i * SCREENWIDTH + 104] = 0;
            screens[0]/*I_VideoBuffer*/[i * SCREENWIDTH + 105] = 0;
        }
    ...
    {
        oldarmor = temp;
        V_DrawPatch(255, 178, 0, PatchARMCLEAR);
...
void DrawInventoryBar(void)
{
    ...
    x = inv_ptr - curpos;
    UpdateState |= I_STATBAR;
    V_DrawPatch(38, 162, 0, PatchINVBAR);
    for (i = 0; i < 7; i++)
    {
    ...
    V_DrawPatch(50 + curpos * 31, 163, 0, PatchSELECTBOX);
/*
    if (x != 0)
    {
        V_DrawPatch(42, 163, 0, !(leveltime & 4) ? PatchINVLFGEM1 :
                    PatchINVLFGEM2);
    }
*/
    if (CPlayer->inventorySlotNum - x > 7)
    {
        V_DrawPatch(269, 163, 0, !(leveltime & 4) ? PatchINVRTGEM1 :
                    PatchINVRTGEM2);
    }
}
...
static void DrawWeaponPieces(void)
{
    if (CPlayer->pieces == 7)
    {
        V_DrawPatch(190, 162, 0, PatchWEAPONFULL);
        return;
    }
    V_DrawPatch(190, 162, 0, PatchWEAPONSLOT);
    if (CPlayer->pieces & WPIECE1)
    {
        V_DrawPatch(PieceX[PlayerClass[consoleplayer]][0], 162, 0, PatchPIECE1);
    }
    if (CPlayer->pieces & WPIECE2)
    {
        V_DrawPatch(PieceX[PlayerClass[consoleplayer]][1], 162, 0, PatchPIECE2);
    }
    if (CPlayer->pieces & WPIECE3)
    {
        V_DrawPatch(PieceX[PlayerClass[consoleplayer]][2], 162, 0, PatchPIECE3);
    }
}
...
*/
    ...
    if(ninty == 1)
    {
	V_DrawPatch(13, 170, 0, PatchINVHBAR);

//	if(oldweapon != CPlayer->readyweapon || manaVialPatch1)
	{ // Update mana graphics based upon mana count/weapon type

	    if(CPlayer->readyweapon == WP_FIRST)
	    {
		manaVialPatch1 = PatchMANAVIALDIMHBAR1;
		manaVialPatch2 = PatchMANAVIALDIMHBAR2;
	    }
	    else if(CPlayer->readyweapon == WP_SECOND)
	    {
		manaVialPatch1 = PatchMANAVIALHBAR1;
		manaVialPatch2 = PatchMANAVIALDIMHBAR2;
	    }
	    else if(CPlayer->readyweapon == WP_THIRD)
	    {
		manaVialPatch1 = PatchMANAVIALDIMHBAR1;
		manaVialPatch2 = PatchMANAVIALHBAR2;
	    }
	    else
	    {
		manaVialPatch1 = PatchMANAVIALHBAR1;
	    ...
	    V_DrawPatch(15, 171, 0, manaVialPatch1);
									// HACK: and that's the trick:
	    for(i=55040;i<55062-(22*CPlayer->mana[0])/MAX_MANA;i++)	// in order to flip the DIM
	    {								// bars, we need to use the
		screens[0]/*I_VideoBuffer*/[i * 1 + 16] = 0;		// pixel length of the hole
		screens[0]/*I_VideoBuffer*/[i * 1 + 336] = 0;		// screen, so these large
	    }								// numbers "MOVE" the line
									// that has been drawn at pixel
	    V_DrawPatch(15, 175, 0, manaVialPatch2);			// 0x0, OVER the edge of the
									// visible screen into the next
	    for(i=56384;i<56406-(22*CPlayer->mana[1])/MAX_MANA;i++)	// pixel line (1x0) and after
	    {								// that we still need to move
		screens[0]/*I_VideoBuffer*/[i * 1 - 48] = 0;		// this line INTO the actual
		screens[0]/*I_VideoBuffer*/[i * 1 + 272] = 0;		// place where it is needed,
	    }								// which is done here by using
									// 16 pixels.
	    oldweapon = CPlayer->readyweapon;
	    UpdateState |= I_FULLSCRN;
	}
    ...
    {
	temp = CPlayer->mana[1];
	DrINumber(temp, 45, 177);
	temp = CPlayer->mana[0];
	DrINumber(temp, 45, 189);
    }
    ...
    else
    {
        ...
        V_DrawPatch(50 + curpos * 31, 167, 0, PatchSELECTBOX);
/*
        if (x != 0)
        {
            V_DrawPatch(40, 167, 0, !(leveltime & 4) ? PatchINVLFGEM1 :
                        PatchINVLFGEM2);
        }
*/
        if (CPlayer->inventorySlotNum - x > 7)
        {
            V_DrawPatch(268, 167, 0, !(leveltime & 4) ?
